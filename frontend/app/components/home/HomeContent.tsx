...
function renderLayoutContent(layout: string) {
  ...
  return (
    <motion.div 
      variants={containerVars} 
      // UPDATED: 
      // 1. Added 'overflow-hidden' to prevent elements sticking out to the right
      // 2. Adjusted padding for better mobile vertical spacing
    ...
    >
      ...
      {/* UPDATED: Changed min-h to fit mobile screens better */}
      <motion.div variants={itemVars} className="col-span-12 lg:col-span-6 relative flex justify-center items-center min-h-[400px] md:min-h-[500px]">
        ...
        {[1, 2, 3].map((index) => (
          <motion.div
            ...
            className="absolute w-full max-w-xs md:max-w-md aspect-square rounded-full border border-gold-primary pointer-events-none z-0"
          />
        ))}
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden flex justify-center items-center">
          <motion.div
            ...
            className="w-[150px] md:w-[200px] h-[300px] bg-gold-primary/10 blur-[60px] rounded-full"
        ...
        <motion.div 
          ...
          // UPDATED: Changed w-[115%] to w-[105%] on mobile to prevent horizontal scroll
          className="absolute w-[105%] md:w-[115%] aspect-square border border-dashed border-gold-primary/20 rounded-full pointer-events-none z-10"
        />
        ...
        <div className="relative group w-full max-w-xs md:max-w-md aspect-square z-20">
          ...
          {/* Floating "Aged" Badge */}
          {/* UPDATED: Adjusted positioning (-right-2) so it doesn't get cut off on mobile */}
          <motion.div 
            animate={{ y: [0, -8, 0] }}
            transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
            className="absolute -top-4 -right-2 md:-right-4 z-40 bg-gold-primary text-home-bg w-20 h-20 md:w-24 md:h-24 rounded-full shadow-2xl flex flex-col items-center justify-center border-4 border-home-bg"
          >
          ...
          {/* UPDATED: Changed -bottom-16 to -bottom-12 to fit tighter on mobile */}
          <div className="absolute -bottom-12 md:-bottom-16 left-0 right-0 text-center">
            <p className="text-[8px] md:text-[9px] uppercase tracking-[0.5em] md:tracking-[0.7em] text-gold-primary font-black">
      ...
      {/* UPDATED: Added px-4 to ensure text doesn't touch screen edges on small mobiles */}
      <motion.div variants={itemVars} className="col-span-12 lg:col-span-6 space-y-8 md:space-y-10 text-center lg:text-left z-20 px-2 md:px-0">
        <div className="space-y-4">
          <h3 className="text-gold-primary tracking-[0.5em] md:tracking-[1em] uppercase text-[9px] md:text-[10px] font-black opacity-70">
            Heritage & Alchemy
          </h3>
          
          {/* UPDATED: Typography scaling. text-5xl on mobile, text-8xl on desktop. */}
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-serif text-home-text leading-[1] md:leading-[0.9]">
            A Decade <br/> 
        ...
        <div className="space-y-8 max-w-lg mx-auto lg:mx-0">
          {/* UPDATED: Reduced font size slightly for mobile readability */}
          <p className="text-home-subtext font-light leading-relaxed text-lg md:text-2xl italic border-l-0 md:border-l-2 border-gold-primary/30 md:pl-8">
          ...
          <div className="grid grid-cols-2 gap-y-10 pt-8 border-t border-gold-primary/10">
            <div>
              ...
              <p className="text-[10px] md:text-xs uppercase tracking-widest text-gold-primary font-bold">Wild Cambodian Oud</p>
            </div>
            <div>
              <p className="text-[8px] uppercase tracking-[0.3em] font-black opacity-40 mb-2">Longevity</p>
              <p className="text-[10px] md:text-xs uppercase tracking-widest text-gold-primary font-bold">Infinite (24h+)</p>
            </div>
          </div>
        </div>

        <div className="pt-4 md:pt-8">
          <button className="group relative px-10 md:px-14 py-5 md:py-6 bg-transparent border border-gold-primary/30 overflow-hidden cursor-pointer">
            <span className="relative z-10 text-gold-primary text-[9px] md:text-[10px] uppercase tracking-[0.5em] md:tracking-[0.8em] font-black group-hover:text-home-bg transition-colors duration-500">
              Acquire Ritual
            </span>
